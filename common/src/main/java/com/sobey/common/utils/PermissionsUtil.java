...
//                        Snackbar snackbar = Snackbar.make(activity.findViewById(src), "这次是真的需要去授权了", Snackbar.LENGTH_INDEFINITE);
...
//            int grantCode = ActivityCompat.checkSelfPermission(activity, permission);
            int grantCode = selfPermissionGranted(activity, permission);
            if (grantCode == PackageManager.PERMISSION_DENIED) {
                denidArray.add(permission);
            }
        }
    ...
    public static int selfPermissionGranted(Context context, String permission) {
